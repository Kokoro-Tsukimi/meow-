...
// F.1.8 #30 修法:用 localStorage 持久化"冷却结束时刻",刷新/重开都能恢复倒计时。
// 与 ForgotPassword 共用同一 key 前缀,因为后端 cooldown 本来就是共用的同一 Redis key——
// 用户在此页发完码 60s 内切到 /forgot-password 也会被前端挡住,体验一致喵。
...
export default function Register() {
  ...
  // #30 修法:从 localStorage 恢复冷却倒计时(挂载时 + email 变化时都检查)
  useEffect(() => {
  ...
  const handleSendCode = async () => {
    ...
    setSendingCode(true);
    try {
      const res = await apiClient.post('/api/v1/auth/send-verify-code', { email: email.trim() });
      setCodeSentMsg(res.data.message || '验证码已寄出,请查收喵~');
      const targetMs = Date.now() + 60 * 1000;
      localStorage.setItem(COOLDOWN_KEY_PREFIX + email.trim(), String(targetMs));
      startCooldown(60);
    } catch (err: any) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!inviteCode.trim() && !verifyCode.trim()) {
      setError('请填写邮箱验证码,或改用邀请函CDK喵~');
      return;
    }

    setSubmitting(true);
    console.info(`[USER-PORTAL][Register][Submit] email: ${email}, 邀请码: ${inviteCode ? '有' : '无'}`);
    ...
    try {
      ...
      setSuccessMsg(res.data.message || '注册成功，欢迎来到喵咖书店~');

      setTimeout(() => {
        navigate('/login');
      }, 2500);
    } catch (err: any) {
      setError(err.response?.data?.message || '注册失败，请稍后再试喵');
      setSubmitting(false);
    }
  };
      ...
      <div className="theme-panel-wrapper w-full max-w-md relative z-10 hover:-translate-y-2 transition-transform duration-500 ease-spring group">
        <div className="theme-panel p-6 sm:p-10 relative flex flex-col h-full w-full">
          <div className="text-center mb-8 relative z-10">
            <h1 className="text-3xl font-black tracking-widest theme-text-title mb-2">办一张借阅证</h1>
            <p className="text-sm theme-text-sub opacity-80 font-medium">加入喵咖魔法书店喵~</p>
          </div>
          ...
          {successMsg ? (
            ...
            <form onSubmit={handleRegister} className="space-y-5 relative z-10">
              ...
              <div>
                <input
                  type="text"
                  placeholder="邀请函 CDK（选填）"
                  className="w-full px-5 py-4 theme-input transition-all duration-300 outline-none"
                  value={inviteCode}
                  onChange={(e) => setInviteCode(e.target.value)}
                />
                <p className="text-xs theme-text-sub opacity-60 mt-2 px-1">
                  💌 有店长发的邀请函？填进来,开户咖啡豆直接到账,还不用验证邮箱喵~
                </p>
              </div>
